...
def load_solps_from_mdsplus(mds_server, ref_number):
    ...
    sim.electron_density = conn.get('\SOLPS::TOP.SNAPSHOT.NE').data()
    ...
    poloidal_flux = conn.get('\SOLPS::TOP.SNAPSHOT.FNAX').data()
    radial_flux = conn.get('\SOLPS::TOP.SNAPSHOT.FNAY').data()
    ...
    if not b2_standalone:
        ...
        try:
            neutral_poloidal_flux = conn.get('\SOLPS::TOP.SNAPSHOT.PFLA').data()[:]
    ...
    # Integrated power #
    try:
        linerad = np.sum(conn.get('\SOLPS::TOP.SNAPSHOT.RQRAD').data()[:], axis=0)
    except (mdsExceptions.TreeNNF, TypeError):
        linerad = 0

    try:
        brmrad = np.sum(conn.get('\SOLPS::TOP.SNAPSHOT.RQBRM').data()[:], axis=0)
    except (mdsExceptions.TreeNNF, TypeError):
        brmrad = 0
...
def load_mesh_from_mdsplus(mds_connection, mdsExceptions):
    ...
    r = mds_connection.get('\TOP.SNAPSHOT.GRID:R').data()
    z = mds_connection.get('\TOP.SNAPSHOT.GRID:Z').data()

    vol = mds_connection.get('\SOLPS::TOP.SNAPSHOT.VOL').data()

    # Loading neighbouring cell indices
    neighbix = np.zeros(r.shape, dtype=np.int)
    neighbiy = np.zeros(r.shape, dtype=np.int)

    neighbix[0] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:LEFTIX').data().astype(np.int)
    neighbix[1] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:BOTTOMIX').data().astype(np.int)
    neighbix[2] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:RIGHTIX').data().astype(np.int)
    neighbix[3] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:TOPIX').data().astype(np.int)

    neighbiy[0] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:LEFTIY').data().astype(np.int)
    neighbiy[1] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:BOTTOMIY').data().astype(np.int)
    neighbiy[2] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:RIGHTIY').data().astype(np.int)
    neighbiy[3] = mds_connection.get('\SOLPS::TOP.SNAPSHOT.GRID:TOPIY').data().astype(np.int)
